...
static int isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    /* Need at least 3 bytes: 2 PCI + 1 data.  Some ECUs send DLC=7 (not padded to 8). */
    if (len < 3) {
        isotp_user_debug("First frame too short.");
        return ISOTP_RET_LENGTH;
    }
    ...
    /* Copy available data bytes from FF (may be less than 6 if DLC < 8) */
    uint8_t ff_data_bytes = (len > 2) ? (len - 2) : 0;
    if (ff_data_bytes > sizeof(message->as.first_frame.data)) {
        ff_data_bytes = sizeof(message->as.first_frame.data);
    }
    (void) memcpy(link->receive_buffer, message->as.first_frame.data, ff_data_bytes);
    /* Zero-fill any missing bytes so buffer is clean */
    if (ff_data_bytes < sizeof(message->as.first_frame.data)) {
        (void) memset(link->receive_buffer + ff_data_bytes, 0,
                      sizeof(message->as.first_frame.data) - ff_data_bytes);
    }
